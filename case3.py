...
# Descripción: Caso 3 - Transforma archivos Excel del Caso 1 en plantillas estándar por cuenta
...
class Case:
    """Caso 3 - Genera plantillas estándar desde archivos formateados del Caso 1."""
    ...
    def __init__(self) -> None:
        self.name = "Caso 3"
        self.description = (
            "Recibe archivos Excel generados por el Caso 1, identifica la cuenta bancaria "
            "y transforma su contenido en plantillas estándar usando la configuración "
        ...
        self.missing_rows_message = (
            ...
            "Por favor, envía un archivo que haya sido previamente procesado por el Caso 1 y que "
            "contenga las filas requeridas con las marcas 'CP' y/o 'CB' en la columna 'Revisar'.\n\n"
            "Saludos cordiales."
        )
    ...
    def get_search_keywords(self) -> List[str]:
        """Obtiene la palabra clave configurada para el caso 3"""
        try:
            config = self.config_manager.load_config()
            search_params = config.get('search_params', {})
            keyword = search_params.get('caso3', '').strip()
            if keyword:
                return [keyword]
            return []
        except Exception as exc:
            print(f"Error al cargar palabras clave para caso3: {exc}")
            return []
    ...
    def _create_template_workbooks(self, attachment: Dict[str, Any], logger) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            account_name = self.config_manager.find_account_by_code(account_code)
            ...
            account_config = self.config_manager.get_case3_account_config(account_name)
    ...
    def _extract_data_rows(
        ...
        """Extrae las filas de datos con 'CP' o 'CB' en la columna Revisar"""
        ...
        if not review_column:
            logger.log(
                f"No se encontró la columna 'Revisar'. Columnas disponibles: {list(header_map.keys())}",
                level="WARNING",
            )
            return None
        ...
        for row_idx in row_range:
            ...
            if is_xls:
                ...
                if code_column:
                    ...
                    code_value = str(code_val).strip() if code_val else ''
            else:
                ...
                if code_column:
                    ...
                    code_value = str(code_cell_value).strip() if code_cell_value is not None else ''
