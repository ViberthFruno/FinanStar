...
# Descripción: Caso 8 - Genera un reporte verde de transacciones sin columna de débitos ni resumen adicional
...
class Case:
    """Caso 8 - Rediseña el estado de cuenta en un formato verde sin totales de débitos."""
    ...
    def __init__(self) -> None:
        ...
        self.description = (
            ...
            "con encabezado verde, omitiendo la columna de débitos y sin generar un resumen adicional."
        )
        self.response_message = (
            "Hola,\n\nSe adjunta el archivo con el formato verde actualizado del estado de cuenta "
            "correspondiente al Caso 8. Quedo atento a cualquier comentario.\n\nSaludos cordiales."
        )
    ...
    def get_search_keywords(self) -> List[str]:
        try:
            ...
            keyword = search_params.get('caso8', '').strip()
            if keyword:
                return [keyword]
            return []
        except Exception as exc:
            print(f"Error al cargar palabras clave para caso8: {exc}")
            return []
    ...
    def _get_removal_configuration(self) -> Tuple[List[str], List[Tuple[str, str]]]:
        configured = self.config_manager.get_case8_columns_to_remove()
    ...
    def _get_codification_rules(self) -> Dict[str, List[Tuple[str, str]]]:
        """Obtiene y prepara las reglas de codificación para el Caso 8."""
        raw_rules = self.config_manager.get_case8_codification_rules()
    ...
    def _highlight_rows_by_filters(
        ...
        """Resalta filas que coincidan con los filtros configurados para el Caso 8."""
        filters = self.config_manager.get_case8_filters()
        if not filters:
            return
        ...
        if not description_column:
            logger.log(
                "No se encontró una columna de descripción para aplicar filtros del Caso 8.",
                level="WARNING",
            )
            return
        ...
        if highlighted_rows:
            logger.log(
                (
                    "Se resaltaron "
                    f"{highlighted_rows} fila(s) que coinciden con los filtros configurados del Caso 8."
                ),
                level="INFO",
            )
    ...
    def _extract_table_rows(self, worksheet, logger) -> List[Dict[str, Any]]:
        ...
        if not header_row or not header_map:
            logger.log(
                "No se encontraron encabezados válidos en el archivo fuente para Caso 8.",
                level="ERROR",
            )
            return []
        ...
        if missing:
            logger.log(
                "No se localizaron todas las columnas requeridas para Caso 8: " + ', '.join(missing),
                level="WARNING",
            )
    ...
    def _build_output_filename(self, original_name: str) -> str:
        ...
        return f"{base}_caso8_{timestamp}.xlsx"
